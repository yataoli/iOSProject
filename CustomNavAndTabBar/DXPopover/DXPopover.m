...
- (void)commonInit {
    ...
    self.arrowSize = CGSizeMake(11.0, 9.0);//CGSizeMake(11.0, 9.0)
//    self.cornerRadius = 5.0;//图片圆角
    self.backgroundColor = [UIColor whiteColor];
    self.animationIn = 0.4;//0.4展开时间
    self.animationOut = 0.0;//0.3缩回时间
    self.animationSpring = YES;
    self.sideEdge = 10.0;
    self.maskType = DXPopoverMaskTypeBlack;
    self.betweenAtViewAndArrowHeight = 0.0;//4.0
...
- (void)show {
    ...
    self.transform = CGAffineTransformMakeScale(1.0, 0.0);//(0.0, 0.0)
...
- (void)dismiss {
    if (self.superview) {
        [UIView animateWithDuration:self.animationOut
            ...
            animations:^{
                self.transform = CGAffineTransformMakeScale(1, 0.0001);//(0.0001, 0.0001)
            }
            completion:^(BOOL finished) {
