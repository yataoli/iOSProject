...
self.typeTableView.frame = CGRectMake(0, 0, 40, 44*6);
self.typeTableView.dataSource = self;
self.typeTableView.delegate = self;
self.typePopover = [DXPopover new];
self.typePopover.maskType = DXPopoverMaskTypeBlack;
//    self.typePopover.arrowSize = CGSizeMake(0.0, 0.0);//去掉箭头
 
 
 
 #pragma mark - 弹出的按钮点击事件
 - (IBAction)selecteButtonClick:(UIButton *)sender {
 
 CGRect tableViewFrame = self.typeTableView.frame;
 tableViewFrame.size.width = sender.frame.size.width;
 self.typeTableView.frame = tableViewFrame;
 self.typePopover.contentInset = UIEdgeInsetsZero;
 ...
 [self.typePopover showAtPoint:startPoint
 popoverPostion:DXPopoverPositionDown
 withContentView:self.typeTableView
 inView:self.view];//_touZhuBottomView
 
 
 self.typePopover.didDismissHandler = ^{
